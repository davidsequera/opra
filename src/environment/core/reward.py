...
class RegularizedSLARewardFunction(SLARewardFunction):
    ...
    Positive rewards are scaled by (beta * (Prob(A) - 1) + 1), where
    ...
    beta=1, Prob(A)=1  →  multiplier=1 (action aligns with original dist).
    beta=1, Prob(A)=0  →  multiplier=0 (action impossible in original dist).
    ...
    def compute(self, ctx: CaseRewardContext) -> float:
        ...
        if base > 0:
            multiplier = self.beta * (ctx.chosen_activity_prob - 1.0) + 1.0
